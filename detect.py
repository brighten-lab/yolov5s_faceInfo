...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # LOGGER.info(f"{s}{''  len(det) else '(no detections), '}{dt[1].dt * 1E3:.1f}ms") # ex) {s} -> 0: 480x640 1 hyemin, 1 / {dt[1].dt * 1E3:.1f} -> 15.1 / len(det) -> 인식하면 1, 아니면 0
        
        if(len(det) != 0): # 사람 인식할때
